...
| Flutter/原生通信 | Spike 用 MethodChannel，Alpha 用 Pigeon |
| 数据策略 | 本地优先，无账号、无服务端 |
...
- 在 UI 层直接调用 Android API；
- 在 Widget 中实现规则判断；
- 依赖 Flutter Timer 作为核心拦截计时。
...
- API 版本化；
- 明确错误码；
- 不直接传递 Android Framework 对象；
- 所有时间统一使用 UTC 毫秒或明确的本地时区语义；
- 大数据量不通过频繁 MethodChannel 调用。
...
AccessibilityService 只读取快照，不直接访问复杂 Flutter 数据库。
...
sequenceDiagram
    ...
    participant DB as Flutter Local DB
    participant B as Pigeon Bridge
...
- Flutter 正在修改规则，AccessibilityService 同时读取；
- 多个无障碍事件同时到达；
- 干预 Activity 重复启动；
- 通行到期与用户主动结束同时发生；
- App 进程和 AccessibilityService 进程状态不同。
...
| PN-PERM-003 | 通知权限未开启 |
| PN-BRIDGE-001 | Flutter/Native 通信失败 |
...
环境、仓库、文档和最小 Flutter 应用可运行。

### 任务

- 安装 Flutter/Android Studio/JDK；
- 初始化 Git；
- 创建 Flutter Kotlin 项目；
- 确定 Application ID；
- 加入 lint、测试和 docs；
- 默认应用真机运行；
- 建立三份基础文档。
...
- Flutter 提高产品 UI 开发速度；
- Kotlin 直接接入系统 API；
- 未来 iOS 可复用部分 UI/领域逻辑；
- 原生控制引擎保持独立可靠。
...
- Flutter Android 构建：<https://docs.flutter.dev/deployment/android>
- Flutter 版本：<https://docs.flutter.dev/release/whats-new>
